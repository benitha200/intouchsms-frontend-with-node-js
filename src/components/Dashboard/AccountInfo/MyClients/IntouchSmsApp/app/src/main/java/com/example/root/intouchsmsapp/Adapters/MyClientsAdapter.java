...
public class MyClientsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {

    private final int VIEW_ITEM = 1;
    private final int VIEW_PROG = 0;

    //private Context mContext;
    private List<MyClientsItems> customersItemsList;
    ...
    public MyClientsAdapter(OnLoadMoreListener onLoadMoreListener, CustomersAdapterListener listener) {
    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        if (holder instanceof MyViewHolder) {
            MyClientsItems myCustomersItems = (MyClientsItems) customersItemsList.get(position);
            Double customerbal = (Double.parseDouble(myCustomersItems.getAccountbal()) * 100.0) / 100.0;
            ...
            ((MyViewHolder) holder).lastlogin.setText("Account Bal "+ NumberFormat.getInstance().format(Math.round(customerbal * 100.0) / 100.0).toString() +
                    " "+"Cr."+" Last Login "+myCustomersItems.getLastlogin());
    ...
    public List<MyClientsItems> getItems() {
    ...
    public void addAll(List<MyClientsItems> lst){
        customersItemsList.clear();
        customersItemsList.addAll(lst);
        notifyDataSetChanged();
    }

    public void addItemMore(List<MyClientsItems> lst){
